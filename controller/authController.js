...
exports.login = catchAsync(async (req, res, next) => {
  ...
  // 2) check if user exist and password is correcrt
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser) {
    return next(
      new AppError('the user belong to the token does no loger exist.', 401),
    );
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: 'Your password rest token( Valid only for 10 min)',
      message,
    });
    res.status(200).json({
      status: 'success',
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordConfirm = req.body.passwordConfirm;
  await user.save();
  //use.findByIdAndUpdate will not work indeed!
